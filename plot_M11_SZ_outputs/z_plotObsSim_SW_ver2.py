...
dir_sim = 'R:\\40715-013 UKFPLOS\\Data\\H&H_Data\\calibration_validation_stats\\val_0608_MixCoef\\flow_stats\\sim_flows'
dir_table = 'R:\\40715-013 UKFPLOS\\Data\\H&H_Data\\calibration_validation_stats\\val_0608_MixCoef\\flow_stats\\sim_flows'
...
# change
####
sim = pd.read_csv('Val_0626_MixCoefDetailedTS_M11_all.csv')
print(sim)
...
for ss in stn:
    ...
    # change
    ####
    obs = obs[(obs['datetime'] >= '2011-09-29') & (obs['datetime'] <= '2011-10-27')]
    sim_dat = sim_dat[(sim_dat['datetime'] >= '2011-09-29') & (sim_dat['datetime'] <= '2011-10-27')]

    # interpolate
    # output = [i for i in range(1, len(sim_dat['datetime']) + 1)]
    # print(len(obs['Obs']), len(output))
    # print(output)


    # spl = make_interp_spline(output, sim_dat['Sim'], k =7)
    # y_smooth = spl(output)
    ...
    # plt.plot(sim_dat['datetime'], y_smooth, label = 'Smoothened', color = 'black', lw = 2)
