...
Updated: take daily mean - take weekly mean of the daily average
and get volume in kilo-acre-ft
...
for ss in stn:
    ...
    obs_agg.set_index(obs_agg['datetime'], inplace = True)
    print(obs_agg)
    obs_agg = pd.DataFrame(obs_agg.iloc[:,1].resample('W').mean())
    print(obs_agg)
    ...
    sim_agg = pd.DataFrame(sim_dat.iloc[:,1].resample('D').mean())
    print(sim_agg)

    # get weekly mean
    sim_agg.reset_index(inplace = True) 
    sim_agg.set_index(sim_agg['datetime'], inplace = True)
    sim_agg = pd.DataFrame(sim_agg.iloc[:,1].resample('W').mean())
    print(sim_agg)   
    ...
    for ii in range(len(obs_agg['datetime'])):
        ...
        # convert cubic feet to kilo acre feet
        obs_agg['vol'][ii] = obs_agg['Obs'][ii-1] * obs_agg['time_dff_sec'][ii]*0.0000229568*1000

    print(obs_agg)
    
    # calculate cumulative sum - by aggregating weekly
    # aggregate sim timeseries
    obs_agg.set_index(obs_agg['datetime'], inplace = True)
    obs_agg = pd.DataFrame(obs_agg.iloc[:,3].resample('W').mean())
    print(obs_agg)
    ...
    for ii in range(len(sim_agg['index'])):
        ...
        # convert cubic feet to kilo-acre-feet
        sim_agg['vol'][ii] = sim_agg['Sim'][ii-1] * sim_agg['time_dff_sec'][ii]*0.0000229568*1000
    ...
    # aggregate sim timeseries
    sim_agg.set_index(sim_agg['datetime'], inplace = True)
    sim_agg = pd.DataFrame(sim_agg.iloc[:,4].resample('W').mean())
    print(sim_agg)
    ...
    plt.ylabel('Cumulative Volume [kilo-acre-feet]')
