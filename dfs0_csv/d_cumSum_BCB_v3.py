...
Updated: this script uses the newly extracted flows from the res1D
...
dir_sim = 'R:\\40715-010\\Data\\calibration_stats\\bcb_stat_summary\\0720\\Structure Flows'
...
for ss in stn:
    ...
    os.chdir(dir_sim)
    sim_dat = pd.read_csv("COCO1_summed_v3.csv")
    sim_dat = sim_dat[['datetime_modified', 'total']]
    sim_dat['datetime_modified'] = pd.to_datetime(sim_dat['datetime_modified'])
    # print(sim_dat)
    ...
    obs.to_csv(ss + "_BK_acc_res1D_analysis.csv")
    ...
    sim_dat.to_csv(ss + "_sim_acc_res1D_analysis.csv")
